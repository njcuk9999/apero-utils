...
print("Downloading %d files and their calibration cascades (trying with %s mode)." % (len(rawframes.to_table()), mode_requested))
...
for raw in rawframes:
    ...
    #  ... 2.- GET ITS LIST OF CALIBRATION FILES (without saving it into a file):
    # ---------------------------------------
    calselector_url = "http://archive.eso.org/calselector/v1/associations?dp_id=%s&mode=%s&responseformat=votable" % (raw["dp_id"].decode(), mode_requested)
