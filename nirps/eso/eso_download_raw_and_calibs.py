#!/usr/bin/env python
# ------------------------------------------------------------------------------------
# Name: eso_download_raw_and_calibs.py
# Version: 2019-11-21
...
# -------
# Executing the script you will get a WARNING from astropy:
...
def download_asset(url, filename=None):
...
print()
print("Looking for SCIENCE frames belonging to the 60.A-9109(A) observing run.")
print("Querying the ESO TAP service at %s" % (ESO_TAP_OBS))

query = """SELECT *
FROM dbo.raw
WHERE prog_id='60.A-9109(A)'
AND dp_cat='SCIENCE'
AND date_obs LIKE '%2021-09-26T09%' """
...
print("Downloading the non-proprietary files (%d files) and their calibration cascades (trying with %s mode)." % (
...
for raw in rawframes:
    ...
    # ---------------------------------------
    sci_url = "http://archive.eso.org/datalink/links?ID=ivo://eso.org/ID?%s&eso_download=file" % raw["dp_id"]
