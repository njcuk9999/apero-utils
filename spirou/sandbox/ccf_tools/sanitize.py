...
def sigma(im):
    return (np.nanpercentile(im, 0.841344746068543)
            - np.nanpercentile(im, 0.15865525393145702)) / 2


def lowpassfilter(v, w):
    """
    low-pass filtering of vector v using a running median of width w.
    This code differs from medfilt in proprely handling NaNs
    """
    v3 = []
    i3 = []
...
def sanitize(
    ...
    """Sanitize
    Program to create *sanitized* files, where outlying points are replaced
    with the model.

    Args:
        files (list of str): Files to be sanitized, needs to be tcorr.
            E.g.: files = glob.glob('Gl699/2*o_pp_e2dsff_tcorr_AB.fits')
    blaze_file (str): Path of the blaze file, for example
        blaze_file = 'varia/2019-04-20_2400404f_pp_blaze_AB.fits'

    model_s1d_file (str): Path of s1d_v model file for the object.
        E.g.: model_s1d_file = 'varia/Template_s1d_Gl699_sc1d_v_file_AB.fits'
    prob_bad (float): probability that a point in a spectrum is bad. Use
        default if you don't have a good reason to modify. Default: 1e-3
    force (bool): Force rewriting files. Useful if you modify the code.
        Default: False
    doplot (list of int): show debug plots for given orders. To do no plots,
        use order that does not exist (any number not in [0, 49)).
        Default: [-1]
    verbose (bool): Print debugging messages Default: False.
    """
    ...
    template = InterpolatedUnivariateSpline(
            wave_template[g],
            flux_template[g],
            k=1,
            ext=1
            )
    mask_template = InterpolatedUnivariateSpline(
            wave_template,
            np.isfinite(flux_template),
            k=1,
            ext=1)
    ...
    for i in (range(len(files))):
        ...
        # loop through files
        if verbose:
            print('Sanitizing file #{0} in {1} -> {2}'
                  .format(i+1, len(files), files[i])
                  )
        ...
        # we project the template onto the wavelength grid and mask pixels for
        # which we do not have a reliable template
        ...
        for iord in range(0, 49):
            ...
            if not np.max(np.isfinite(data[iord])):
                data[iord] = np.nan
                continue
            ...
            if iord in doplot:
                ...
                ax[1].legend()
                ax[0].plot(wave[iord], nsig, label='nsig  before', alpha=0.8)
                ax[0].plot(wave[iord],
                           nsig_after,
                           label='nsig after',
                           alpha=0.8)

                ax[2].plot(wave[iord][min_valid:max_valid],
                           pp[min_valid:max_valid],
                           color='green',
                           label='prob')
                ax[2].legend()
            ...
            if np.isfinite(fgood[iord]) == 0:
                fgood[iord] = 0
            hdr['PSANI'+str_iord] = (fgood[iord],
                                     'Order {0} mean(p) sanitize'.format(iord)
                                     )
        ...
        prob_map[~np.isfinite(prob_map)] = 0
        ...
        # we also write the e2ds file for the model. This can be used as a
        # sanity check and to determine telluric correction accuracy.
        fits.writeto(
                'model'.join(outname.split('sani')),
                model,
                hdr,
                overwrite=True
                )
...
# Run this only if directly executed
