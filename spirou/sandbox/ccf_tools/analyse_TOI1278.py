...
# do not *formally* exclude an order,
# but this is done later with the bandpass keyword
exclude_orders = [-1]
...
mask = 'gl846_neg'
method = 'all'
nMAD_cut = 5  # Number of MADs for sigma clipping

tbl = get_object_rv(object, mask=mask,
                    ccf_parent=path_to_ccfs,
...
tbl_bin = per_epoch_table(tbl, nMAD_cut=nMAD_cut)
...
print('Amplitude of the sinusoidal at {0} days: {1:.2f} m/s'
      .format(period, 1000*fit[1]))
print('Mean velocity: {:.2f} m/s'.format(1000*fit[2]))

print('Mean/Median per-epoch STDDEV {0}/{1} km/s'
      .format(np.mean(tbl_bin["ERROR_RV"]), np.median(tbl_bin["ERROR_RV"])))

fig, ax = plt.subplots(nrows=2, ncols=1, sharex=True, figsize=(14, 8))

for i in range(len(t2)):
    ax[0].plot_date(t2.plot_date, tbl_bin['RV'], 'g.')
    ax[0].plot_date([t2[i].plot_date, t2[i].plot_date],
                    [tbl_bin['RV'][i]-tbl_bin['ERROR_RV'][i],
                     tbl_bin['RV'][i]+tbl_bin['ERROR_RV'][i]],
                    'g')
ax[0].plot_date(t3.plot_date, tbl['RV'], 'r.', alpha=0.5)
ax[1].errorbar(t3.plot_date,
               tbl['RV']-model, yerr=tbl['ERROR_RV'],
               linestyle="None", fmt='o', color='green', alpha=0.2,
...
ax[1].legend()
ax[1].plot(Time(time_plot, format='mjd').plot_date,
           np.zeros(len(time_plot)),
           'r:')
...
print('stddev(obs-model) {0:.2f} m/s, '
      'mean ERROR_RV {1:.2f} m/s, median ERROR_RV {2:.2f} m/s, '
      'reduced chi2 {3:.2f} '
      .format(sigma*1e3, mean_error*1e3, median_error*1e3, reduced_chi2))
...
print(' mean ERROR_RV {0:.2f} m/s, median ERROR_RV {1:.2f} m/s'
      .format(mean_error*1e3, median_error*1e3))


f = open('TOI1278_obslog.tex', 'w')
# create an observation log in tex format
# Nice when you want to write a paper in the end,
# hey, that's the point of all these observations!
for i in range(len(tbl)):
    f.write('{0:.4f} & ${1:.3f} \\pm {2:.3f}$ & {3:.3f} \\\\ \n'.
            format(tbl['MJDATE'][i],
                   tbl['RV'][i],
                   tbl['ERROR_RV'][i],
                   tbl['D2_RESIDUAL_CCF'][i]
                   )
            )
f.close()
