...
def per_epoch_table(tbl_input, nMAD_cut=np.inf):
    ...
    for i in range(len(udates)):
        ...
        tbl_bin['SIG_FROM_MAD'][i] = np.nanmedian(
                np.abs(tbl['RV'][g]-np.nanmedian(tbl['RV'][g]))
                ) / 0.681
        ...
        tbl_bin['FORMAL_SIG'][i] = (tbl_bin['ERROR_RV'][i]
                                    / np.sqrt(tbl_bin['RV_N'][i])
                                    )

        print('For epoch MJD = {0:5.2f}, the biggest outlier is at '
              '{1:.3f} median abs deviation'
              .format(tbl_bin['MJDATE_MEAN'][i], np.max(nMAD))
              )
