...
def mk_harps_template(files, outname, obj, teff,force = False):
    ...
    if os.path.isfile(outname)*(force == False):
        print(et.color('{} exists and Force = False'.format(outname),'red'))
        return
    ...
    #berv = np.zeros_like(files,dtype = float)
    ...
    #berv = np.array(berv,dtype = int)


    #uberv = np.unique(np.round(berv))

    cube = np.zeros([len(grid), len(files)])+np.nan
