...
def lblrv(obj_sci,obj_template = None,doplot_ccf = False,doplot_debug = False, force = False,
    ...
    for ifile in range(len(scifiles)):
        ...
        if failed_convergence:
            ...
            if ('FP' not in obj_sci):
                rv = systemic_all[np.argmin(hdr['MJDATE'] - mjdate_all)] + hdr['BERV']*1000
            else:
                rv = systemic_all[np.argmin(hdr['MJDATE'] - mjdate_all)]
        ...
        while not last_iteration:
            last_iteration =  not ((ite_rv < 10) and (np.abs(rv_mean)>bulk_error/10.0))
            ...
            for i in range(0,len(tbl['ORDER'])):#, leave = False):
                ...
                # we keep track of the second derivative
                if last_iteration:
                    # From Bouchy 2001 equation, RV error for each pixel
            ...
            print('\tstdev_meas/stdev_pred = {0:.4f}'.format(stddev_nsig))
            # we force to one
            dvrms *= stddev_nsig
            ...
            print('Duration of loop iteration : {0}\n'.format( datetime.now()-now))
            ite_rv +=1
        ...
        if not ('FP' in obj_sci):
            systemic_all[ifile] = rv-hdr['BERV']*1000
        else:
            systemic_all[ifile] = rv
        ...
        hdr['ITE_RV'] = ite_rv,'N of iter to reach 0.1 sigma num accuracy'
