...
file = 'Template_s1d_GL382_sc1d_v_file_AB.fits'
...
files = glob.glob('Template*1d*.fits')
files.sort()

all_stars = Table()
all_stars['NAME'] = np.zeros_like(files,dtype = '<U99')
all_stars['TEFF'] = np.zeros_like(files,dtype = float)
all_stars['FWHM'] = np.zeros_like(files,dtype = float)

for ifile in np.arange(len(files)):

    file = files[ifile]
    print(file,ifile,len(files))
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    print(np.max(ccf) - np.min(ccf))
    """
    if (np.max(ccf) - np.min(ccf))<0.02:
    ...
    print(' FWHM : {:.3f} km/s'.format(fwhm/1000))
    ...
    all_stars['NAME'][ifile] = hdr['OBJECT']
    all_stars['TEFF'][ifile] = hdr['OBJTEMP']
    all_stars['FWHM'][ifile] = fwhm


plt.plot(all_stars['TEFF'],all_stars['FWHM'],'go')
