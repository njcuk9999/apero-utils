...
def get_object_info(fpattern, local_file=None, keys=None, verbose=False):
    """
...
def check_gaia(names, df_sheet):
    ...
    inds = np.where(
            dup_gaia_id.values[:, None] == df_sheet.GAIADR2ID.values
            )[-1]
...
def str_match(df_test, df_sheet, min_len=2):
    ...
    matches_full = test_names.apply(
            lambda tn: [obj for obj in df.index
                        if np.isin(tn, df.ALIASES[obj].split('|'))
                        ]
            )
    mask = (matches_full.str.len() > 0)
    comments = pd.Series(['']*test_names.size, index=test_names.index)
    comments[mask] += (
            'Full: ' + matches_full[mask].str.join(',')
            )
    ...
    matches_alias_in_test = test_names.apply(
            lambda tn: [obj for obj in df.index
                        if bool(re.search(esc_aliases[obj], tn))
                        ]
            )
    mask = (matches_alias_in_test.str.len() > 0)
    comments[mask] += (
            '  Alias in test: ' + matches_alias_in_test[mask].str.join(',')
            )

    # Check if an alias contains a test name
    matches_test_in_alias = test_names.apply(
            lambda tn: [obj for obj in df.index
                        if tn in df.ALIASES[obj] and len(tn) > min_len
                        ]
            )
    mask = (matches_test_in_alias.str.len() > 0)
    comments[mask] += (
            '  Test in alias: ' + matches_test_in_alias[mask].str.join(',')
            )
...
def teff_and_rv(sh, df_sheet, df_loc):
    ...
    for remote_object in df_sheet['OBJECT']:
        ...
        rv, rv_count = ut.vals_and_counts(objrows,
                                          'OBJRV',
                                          threshold=(0, 2000))
        teff, teff_count = ut.vals_and_counts(objrows,
                                              'OBJTEMP',
