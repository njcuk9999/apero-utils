...
def add_lbl_count(profile: dict, object_table: Table,
    ...
    for pos, objname in tqdm(enumerate(objnames)):
        ...
        for lblfilekey in LBL_STAT_FILES:
            # get the glob for this stat file
            lblsfile_glob = LBL_STAT_FILES[lblfilekey].format(objname)
            # get all files using glob
            all_lblsfiles = glob.glob(os.path.join(lbl_path, LBL_STAT_DIR,
                                                   lblsfile_glob))
            # add a list to file dict for this object
            file_dict[objname][lblfilekey] = all_lblsfiles
    # -------------------------------------------------------------------------
    # add to object table
...
class ObjectData:
    def __init__(self, profile, settings, headers,
        ...
        self.lbl_stat_bnames = dict()
        for lblfilekey in LBL_STAT_FILES:
            lblsfiles = file_dict[objname].get(lblfilekey, [])
            self.lbl_stat_files[lblfilekey] = lblsfiles
            self.lbl_stat_bnames[lblfilekey] = []
            for lblsfile in lblsfiles:
                self.lbl_stat_bnames[lblsfile] += [os.path.basename(lblsfile)]
        ...
        for lbl_objtmp in lbl_objtmps:
            ...
            for lblfilekey in LBL_STAT_FILES:
                ...
                if lblsfilet in self.lbl_stat_bnames[lblfilekey]:
                    ...
                    for lblsfile in self.lbl_stat_files[lblfilekey]:
                        # check if the basename matches
                        if os.path.basename(lblsfile) == lblsfilet:
                            # add to the list
                            down_files.append(lblsfile)
                            # add to the list
                            down_descs.append(lblfilekey)
                            # add to the dictionary
                            self.lbl_stat_files[lblfilekey] = lblsfile
                            # break the loop
                            break
