...
#check3: how many of each unique output do we have?
         ...
         #output4: {ODOMETER_CODE}_pp_with-order_C.fits
#stop1: check3 == check1?
#check4: using the log.fits how many files failed one or more QC? Which odometers? Which nights? Which QC?
#check5: plot the different QCs as a function of time. 
#check6: using the log.fits how many files failed to finish? Which odometers? Which nights? Why (using the ERRORS and LOGFILE columns)?
...
#output list
output_list = ['*_pp_order_profile_AB.fits', '*_pp_order_profile_C.fits', '*_pp_loco_AB.fits', '*_pp_loco_C.fits', '*_pp_fwhm-order_AB.fits', '*_pp_fwhm-order_C.fits', '*_pp_with-order_AB.fits', '*_pp_with-order_C.fits']
...
for i in range(len(reduced_nights)):
    ...
    output3_list_files = list_files('{0}/{1}'.format(reduced_path, reduced_nights[i]), files = output_list[2][1:])
    output4_list_files = list_files('{0}/{1}'.format(reduced_path, reduced_nights[i]), files = output_list[3][1:])
    output5_list_files = list_files('{0}/{1}'.format(reduced_path, reduced_nights[i]), files = output_list[4][1:])
    output6_list_files = list_files('{0}/{1}'.format(reduced_path, reduced_nights[i]), files = output_list[5][1:])
    output7_list_files = list_files('{0}/{1}'.format(reduced_path, reduced_nights[i]), files = output_list[6][1:])
    output8_list_files = list_files('{0}/{1}'.format(reduced_path, reduced_nights[i]), files = output_list[7][1:])
...
if num_logfits_QCfalse == 0:
    ...
    inspect_check4 = inspect_table('localisation_test1', 'check4', data_dict_check4, 'Nights that Failed Quality Control')
...
if num_logfits_ENDEDfalse == 0:
    ...
    inspect_check5 = inspect_table('localisation_test1', 'check5', data_dict_check5, 'Nights that Failed to Finish')
...
elif output1_num_unique < recipe_num_logfits or output2_num_unique < recipe_num_logfits or output3_num_unique < recipe_num_logfits or output4_num_unique < recipe_num_logfits or output5_num_unique < recipe_num_logfits or output6_num_unique < recipe_num_logfits or output7_num_unique < recipe_num_logfits or output8_num_unique < recipe_num_logfits:
    
    color_stop1 = 'Yellow'
    result_stop1 = 'No'

    #if missing output
    if len(output1_missing) > 0 or len(output2_missing) > 0 or len(output3_missing) > 0 or len(output4_missing) > 0 or len(output5_missing) > 0 or len(output6_missing) > 0 or len(output6_missing) > 0 or len(output7_missing) > 0 or len(output8_missing) > 0:
        
        comment_stop1 = 'One or more output were not produced.'
        data_dict_stop1 = {'Night': np.concatenate((night_output1_missing, night_output2_missing, night_output3_missing, night_output4_missing, night_output5_missing, night_output6_missing, night_output7_missing, night_output8_missing)),
        'File name': np.concatenate((output1_missing, output2_missing, output3_missing, output4_missing, output5_missing, output6_missing, output7_missing, output8_missing)),
        }
        inspect_stop1 = inspect_table('localisation_test1', 'stop1', data_dict_stop1, 'Missing Outputs in {0}'.format(reduced_path))
    #if duplicates
    else:
        
        comment_stop1 = 'Recipe called 3 times or more in the same night directory.'
        data_dict_stop1 = {'Night': np.concatenate((night_output1_dup, night_output2_dup, night_output3_dup, night_output4_dup, night_output5_dup, night_output6_dup, night_output7_dup, night_output8_dup)),
        'File name': np.concatenate((output1_dup, output2_dup, output3_dup, output4_dup, output5_dup, output6_dup, output7_dup, output8_dup)),
        }
        inspect_stop1 = inspect_table('localisation_test1', 'stop1', data_dict_stop1, 'Localisation Recipe Called 5 Times or More While Producing the Same Four Outputs in {0}'.format(reduced_path))
