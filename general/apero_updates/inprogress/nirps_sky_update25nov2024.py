...
from etienne_tools import hdr2wave, sigma,lowpassfilter
...
if __name__ == "__main__":
    ...
    if fiber == 'HE':
        ...
        waveref = fits.getdata('C7A164F31A_pp_e2dsff_A_wavesol_ref_A.fits')
    ...
    med = np.zeros_like(all_sky_A, dtype=float)
    for i in range(len(all_sky_A)):
        data = np.array(fits.getdata(all_sky_A[i]), dtype=float)
        med[i] = np.nanmedian(data)

    cut = np.nanmedian(med)*5 # suspect sky files have a median of 5 times the median of the good sky files
    all_sky_A = all_sky_A[med < cut]
    all_sky_B = all_sky_B[med < cut]
    ...
    for i in tqdm(range(len(all_sky_A))):
        ...
        for iord in range(nord):
            tmp = np.array(sky_A[iord])
            mask0 = np.ones_like(tmp, dtype=float)
            mask0[tmp == 0] = np.nan

            tmp -= lowpassfilter(tmp*mask0, 101)
            nsig = tmp/sigma(tmp)
            mask = np.ones_like(tmp,dtype=float)
            mask[nsig > 5] = np.nan
            tmp -= lowpassfilter(tmp*mask*mask0, 101)
            sky_A[iord] = tmp*mask0

            tmp = np.array(sky_B[iord])
            mask0 = np.ones_like(tmp, dtype=float)
            mask0[tmp == 0] = np.nan
            tmp -= lowpassfilter(tmp*mask0, 101)
            nsig = tmp/sigma(tmp)
    ...
    for i in range(65):
        plt.plot(waveref2.ravel(), all_sky_map_A[i])
    plt.show()

    wavex = 1282.475 # sky line to check fluorescence
    # id = 173

    # sky lines may be present only part of the time, we therefore take the 90th percentile
    # the 16-84th percentile range is used to estimate the noise.
    # We low-pass the noise as actual lines will increase the noise locally.
    v90 = np.nanpercentile(all_sky_map_A,90, axis=0)

    v90 -= lowpassfilter(v90, 101)
    sig = lowpassfilter(np.abs(v90))
    # find positive excursions in sky signal
    nsig = v90 / sig
    mask = np.ones_like(nsig, dtype=float)
    mask[nsig > 5] = np.nan
    v90 -= lowpassfilter(v90*mask, 101)

    plt.plot(waveref2,5*sig, 'r-')
    plt.plot(waveref2,v90, 'b-')
    plt.show()
    ...
    magic_grid = np.array(Table.read('NIRPS_2023-04-04T07_04_28_544_pp_s1d_v_A.fits')['wavelength'])
    ...
    vv = v90.reshape(waveref.shape)
    for iord in range(waveref.shape[0]):
        plt.plot(waveref[iord], vv[iord], alpha=0.5)
    plt.plot(waveref2[reg_id != 0], v90[reg_id != 0], 'g.', alpha=0.3)
    ...
    model_A = np.zeros_like(v90)
    model_B = np.zeros_like(v90)
    ...
    for ii in tqdm(np.unique(reg_id)[1:]):
        ...
        for i in range(all_sky_map_B.shape[0]):
            ...
            #amp = np.nansum(tmp_A)
            all_A = np.append(all_A, tmp_A)
            all_B = np.append(all_B, tmp_B)

        all_A = all_A.reshape(Nbin, len(tmp_A))
        all_B = all_B.reshape(Nbin, len(tmp_B))


        for fiber_id in range(2):
            if fiber_id == 0:
                all_fib = np.array(all_A)
            else:
                all_fib = np.array(all_B)

            med_fib = np.nanmedian(all_fib, axis=0)
            ...
            for ite in range(2):
                ...
                sig = np.ones_like(all_fib)
                amps_fib = np.zeros(Nbin)
                for i in range(Nbin):
                    amps_fib[i] = np.nansum(all_fib[i] * med_fib)/np.nansum(med_fib**2)
                    sig[i]/=amps_fib[i]
                    all_fib[i] /= amps_fib[i]


                med_fib = np.zeros(all_fib.shape[1])
                for ipix in range(all_fib.shape[1]):
                    med_fib[ipix] = weighted_median(all_fib[:, ipix], 1/sig[:, ipix]**2)

            if fiber_id == 0:
                all_A = np.array(all_fib)
                med_A = np.array(med_fib)
            else:
                all_B = np.array(all_fib)
                med_B = np.array(med_fib)


        if doplot:
            ax[0].plot(waveref2[gg], med_A, alpha=0.5, color='orange')
            ax[1].plot(waveref2[gg], med_B, alpha=0.5, color='orange')
